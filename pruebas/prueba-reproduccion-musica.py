...
class NodoCancion:
    def __init__(self, titulo, artista, duracion, genero="Desconocido", archivo_audio=None):
        ...
        self.genero = genero
        self.archivo_audio = archivo_audio  # Ruta del archivo de audio
        self.siguiente = None
...
class Playlist:
    ...
    def reproducir_audio(self, tiempo_inicio=0):
        """Reproduce el audio de la canción actual desde un tiempo específico"""
        if self.actual and self.actual.archivo_audio:
            try:
                pygame.mixer.music.load(self.actual.archivo_audio)
                pygame.mixer.music.play(start=tiempo_inicio)
    ...
    def obtener_tiempo_actual(self):
        ...
        if self.reproduciendo or self.pausado:
            return pygame.mixer.music.get_pos() / 1000  # Convertir ms a segundos
        return self.tiempo_reproduccion

class SpotifyStylePlayer:
    def __init__(self, root):
        self.root = root
        self.root.title("Interfaz con Audio Real")
        ...
        self.actualizando_progreso = False
        self.setup_styles()
        self.setup_ui()
    ...
    def setup_ui(self):
        ...
        progress_frame = ttk.Frame(controls_frame, style='Light.TFrame')
        progress_frame.pack(fill=tk.X, pady=10)
        
        self.progress_scale = Scale(progress_frame, from_=0, to=100, orient=tk.HORIZONTAL,
                                  bg=self.colors['bg_light'], fg=self.colors['white'],
                                  troughcolor=self.colors['dark_gray'],
                                  sliderlength=15, length=400,
                                  command=self.cambiar_tiempo_reproduccion)
        self.progress_scale.pack(pady=5)
        
        self.tiempo_label = ttk.Label(progress_frame, text="0:00 / 0:00", 
        ...
        self.volumen_scale = Scale(audio_controls, from_=0, to=100, orient=tk.HORIZONTAL,
                                 bg=self.colors['bg_lighter'], fg=self.colors['white'],
                                 troughcolor=self.colors['dark_gray'],
                                 sliderlength=10, length=100,
                                 command=self.ajustar_volumen)
        self.volumen_scale.set(70)  # Volumen por defecto 70%
        self.volumen_scale.pack(side=tk.RIGHT, padx=5)
        
        ttk.Label(audio_controls, text="Volumen:", 
                 style='Subtitle.TLabel', foreground=self.colors['white']).pack(side=tk.RIGHT, padx=5)
    ...
    def cargar_canciones_ejemplo(self):
        # Canciones de ejemplo (necesitarías archivos de audio reales)
        canciones = [
            ("Bohemian Rhapsody", "Queen", 355, "Rock", "audio/bohemian.mp3"),
            ("Blinding Lights", "The Weeknd", 200, "Pop", "audio/blinding.mp3"),
            ("Save Your Tears", "The Weeknd", 215, "Pop", "audio/tears.mp3"),
            ("Stairway to Heaven", "Led Zeppelin", 482, "Rock", "audio/stairway.mp3"),
            ("Shape of You", "Ed Sheeran", 233, "Pop", "audio/shape.mp3"),
        ]
        
        for titulo, artista, duracion, genero, archivo in canciones:
            # Verificar si el archivo existe, sino usar None (reproducción simulada)
    ...
    def actualizar_info_cancion(self, cancion):
        if cancion:
            ...
            self.progress_scale.config(to=cancion.duracion)
            self.progress_scale.set(0)
            self.actualizar_tiempo_label(0, cancion.duracion)
        else:
            self.now_playing_text.set("Selecciona una canción para reproducir")
    
    def actualizar_tiempo_label(self, actual, total):
        """Actualiza la etiqueta de tiempo"""
        min_actual = int(actual) // 60
        seg_actual = int(actual) % 60
        min_total = total // 60
        seg_total = total % 60
        self.tiempo_label.config(text=f"{min_actual}:{seg_actual:02d} / {min_total}:{seg_total:02d}")
    ...
    def cambiar_tiempo_reproduccion(self, value):
        """Cambia el tiempo de reproducción cuando el usuario mueve la barra"""
        if not self.actualizando_progreso and self.playlist.actual:
            tiempo_seleccionado = float(value)
            if self.playlist.reproduciendo or self.playlist.pausado:
                self.playlist.detener_audio()
                if self.playlist.reproducir_audio(tiempo_seleccionado):
                    if self.playlist.pausado:
                        self.playlist.pausar_audio()
                    else:
                        self.play_btn.config(text="⏸")
    ...
    def añadir_cancion(self):
        ...
        tk.Label(dialog, text="Archivo de audio (opcional):", bg=self.colors['bg_light'], fg='white').grid(row=4, column=0, padx=10, pady=5, sticky=tk.W)
        ...
        def guardar():
            try:
                ...
                if archivo_audio and not os.path.exists(archivo_audio):
                    messagebox.showwarning("Advertencia", "El archivo de audio no existe. Se añadirá sin audio.")
                    archivo_audio = None
    ...
    def actualizar_tiempo(self):
        ...
        if self.playlist.actual:
            if self.playlist.reproduciendo:
                tiempo_actual = self.playlist.obtener_tiempo_actual()
                if tiempo_actual >= self.playlist.actual.duracion:
                    self.siguiente()
                else:
                    self.actualizando_progreso = True
                    self.progress_scale.set(tiempo_actual)
                    self.actualizar_tiempo_label(tiempo_actual, self.playlist.actual.duracion)
                    self.actualizando_progreso = False
        
        self.root.after(100, self.actualizar_tiempo)  # Actualizar más frecuentemente
